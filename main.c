...
static void updateMoteRoutingTable(int current_mote, int hopInt, int destInt, int senderMote);
static void showMoteRoutingTable();
static void sendDATA(int dest, char payload);
...
static char payload[18] = "SendSomeRandomData";
...
/********* Functions to manage routing table**********/
// Function to update routing table on RREQ and RREP
static void updateMoteRoutingTable(current_mote, hopInt,destInt, senderMote) {
        if(rrepNextHops[current_mote].valid == 0){
                routingTable[current_mote].next = senderMote;
                routingTable[current_mote].dest = destInt;
                routingTable[current_mote].dist = hopInt;
                rrepNextHops[current_mote].valid = 1;
        }
        showMoteRoutingTable();
}

static void showMoteRoutingTable() {
    int mote, found;
    for(mote=0; mote<MOTES_COUNT; mote++)
    {
        found = 0;
        if(rrepNextHops[mote].valid!= 0)
        {
            printf("\n Routing table entry added on mote %d  {Dest:%d; Next:%d; Hops:%d; Sequence:%d}",
                    mote, 
                    routingTable[mote].dest, routingTable[mote].next,
                    routingTable[mote].dist, routingTable[mote].seq);
            found ++;
            if(found==0) printf("Routing table of mote %d is empty!",mote);
        }
    } 
}
...
static void sendDATA(int dest, char payload){
  ...
  packetbuf_copyfrom(packet, 18); // Copy data to the packet buffer 
    ...
    if(!next_hop || next_hop == 0){
	    ...
	    printf("Forwarded payload %s with destination %.2d to next hop %.2d\n",payload,dest,next_hop);
	    unicast_send(&data_conn, &addr);
    }
}
...
static void receiveRREQ(struct broadcast_conn *c, const linkaddr_t *from) {
    ...
    updateMoteRoutingTable(linkaddr_node_addr.u8[0], hopInt,destInt, from->u8[1]);
...
PROCESS_THREAD(begin_discovery_on_button_click, ev, data) {
    ...
    while(1){
		...
		//TODO - Send Data here
		sendDATA(randomDestinationAddress, payload);
		} else {
